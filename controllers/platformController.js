...
exports.get_platform_list = function(req, res, next){
  Platform.find({})
          ...
          .exec(function(err, platforms){
            ...
            res.render('platform_list', {title: 'Platforms', platform_list: platforms, user: req.session.id});
          });
}
  ...
  async.parallel({
      ...
      games: function(callback){
        Game.find({'platform': req.params.id}, callback)
          ...
          if(err){ return next(err); }
          res.render('platform_detail', {title: 'Platform', platform: results.platform, games_list: results.games, user: req.session.id});
      }
    );
}
...
exports.get_platform_create = function(req, res, next){
  ...
  res.render('platform_form', {title: 'Create Platform', user: req.session.id});
}
...
exports.post_platform_create = [
  ...
  (req, res, next) => {
    ...
    if(!errors.isEmpty()){
      return res.render('platform_form', {title: 'Create Platform', platform: platform, errors: errors.array()})
    } else{
...
exports.get_platform_update = function(req, res, next){
  Platform.findById(req.params.id, function(err, platform){
    if(err){ return next(err); }
    return res.render('platform_form', {title:'Update Platform: ' + platform.name, platform: platform, user: req.session.id});
  });
}
...
exports.post_platform_update = [
  ...
  (req, res, next) => {
    ...
    if(!errors.isEmpty()){
      return res.send('platform_form', {title: 'Create Platform', platform: platform, errors: errors.array(), user: req.session.id})
    } else{
...
exports.get_platform_delete = function(req, res, next){
  async.parallel({
    ...
    return res.render('platform_delete', {title: 'Delete Platform: ', platform: results.platform, games_list: results.games, user: req.session.id});
  });
}

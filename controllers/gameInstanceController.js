...
exports.get_gameinstance_list = function(req, res, next){
  async.parallel({
    gameinstance_list: function(callback){
      ...
      res.render('gameinstance_list', {title: 'Game Instances', gameinstance_list: results.gameinstance_list, game_list: results.games, sort_by: req.query.sort_by, filter_by: req.query.filter_by, user: req.session.user_id});
  });
}
...
exports.get_gameinstance = function(req, res, next){
  GameInstance.findById(req.params.id)
              ...
              .exec(function(err, result){
                ...
                return res.render('gameinstance_detail', {title:"Game Instance", gameinstance: result, user: req.session.user_id});
              });
}
...
exports.get_gameinstance_create = function(req, res, next){
  Game.find({}, function(err, games){
      if(err){ return next(err); }
      res.render('gameinstance_form', {title: 'Create Game Instance', game_list: games, user: req.session.user_id});
  });
}
...
exports.post_gameinstance_create = [
  ...
  (req, res, next) => {
    ...
    if(!errors.isEmpty()){
      Game.find({}, function(err, games){
          if(err){ return next(err); }
          res.render('gameinstance_form', {title: 'Create Game Instance', game_list: games, gameinstance: gameinstance, errors:errors.array(), user: req.session.user_id});
      });
    } else{
...
exports.get_gameinstance_delete = function(req, res, next){
  GameInstance.findById(req.params.id, function(err, gameinstance){
    if(err){ return next(err); }
    return res.render('gameinstance_delete', {title: 'Delete Game Instance', gameinstance: gameinstance, user: req.session.user_id});
  });
};
...
exports.get_gameinstance_update = function(req, res, next){
  async.parallel({
    ...
    if(err){ return next(err); }
    return res.render('gameinstance_form', {title: 'Update Game Instance', gameinstance: results.gameinstance, game_list: results.games, user: req.session.user_id});
  }
)};
